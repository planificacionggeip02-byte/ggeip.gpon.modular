...
function getRegistrosPorRif(rifQuery) {
  // Conservamos la intención original y añadimos normalización robusta
  const normalizeRif = s => (s || '').toString().trim()
    .replace(/[^0-9a-zA-Z]/g, ''); // solo alfanuméricos

  const q = normalizeRif(rifQuery);
  if (!q) return [];

  console.log("🔎 Buscando registros por RIF (normalizado):", q);
  ...
  data.forEach(row => {
    const rifRaw = (pick(row, 'rif') || '').toString();
    const rifCellNorm = normalizeRif(rifRaw);
    if (!rifCellNorm.includes(q)) return;

    // Devolvemos campos clave — usando aliases tolerantes para que aparezcan en la tabla
    resultados.push({
      id_registro: pick(row, 'id_registro', 'id') || '',
      rif: rifRaw || '',
      cliente: pick(row, 'cliente', 'razon_social') || '',
      region: pick(row, 'region') || '',
      estado: pick(row, 'estado') || '',
      contrato: pick(row, 'contrato') || '',
      // Alias tolerantes para número servicio 700
      numero_servicio_700: pick(row,
        'Número Servicio 700', 'Numero Servicio 700',
        'N° Servicio 700', 'Nº Servicio 700',
        'numero_servicio_700', 'nro_servicio_700', 'numero servicio 700') || '',
      // Alias tolerantes para situación actual
      situacion_actual: pick(row,
        'Situacion Actual', 'Situación Actual',
        'situacion_actual', 'situación actual') || ''
    });
  });

  console.log(`✅ ${resultados.length} registro(s) encontrado(s) (getRegistrosPorRif)`);
  return resultados;
}
...
function getRegistroPorIdYFila(idRegistro) {
  idRegistro = (idRegistro || '').toString().trim();
  if (!idRegistro) return null;

  console.log("🔎 getRegistroPorIdYFila solicitado con idRegistro raw:", idRegistro);
  ...
  // Normalizadores
  const idNorm = s => (s || '').toString().trim().toLowerCase();
  const normalizar = h => (h || '').toString().trim().toLowerCase()
    .replace(/\s+/g, '_')
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '');

  const headers = sh.getRange(1,1,1,lastCol).getValues()[0];
  const headersKeys = headers.map(h => normalizar(h));

  // Buscar columna ID (tolerante a variantes)
  let idCol = headersKeys.indexOf('id');
  if (idCol === -1) idCol = headersKeys.indexOf('id_registro');
  if (idCol === -1) throw new Error('No existe la columna "ID" ni "id_registro" en encabezados');

  // Detectar índice de columna teléfono (varias variantes posibles)
  const phoneAliases = ['telefono', 'telefono_completo', 'telefono_completo', 'telefono_completo']; // normalized aliases
  let telefonoIdx = -1;
  for (const alias of phoneAliases) {
    const i = headersKeys.indexOf(alias);
    if (i !== -1) { telefonoIdx = i; break; }
  }

  const rows = sh.getRange(2,1,lastRow-1,lastCol).getValues();
  const requestedNorm = idNorm(idRegistro);

  // Helper: serializar valor (fechas -> yyyy-MM-dd, otros -> string)
  const serialize = v => {
    if (v instanceof Date) {
      // Usa la zona horaria del script si está disponible
      try {
        const tz = Session && Session.getScriptTimeZone ? Session.getScriptTimeZone() : 'UTC';
        return Utilities.formatDate(v, tz, 'yyyy-MM-dd');
      } catch (e) {
        return v.toISOString ? v.toISOString() : String(v);
      }
    }
    // si es fórmula vacía o null -> ''
    if (v === null || v === undefined) return '';
    return typeof v === 'string' ? v : String(v);
  };

  // Primero: búsqueda estricta normalizada
  for (let r = 0; r < rows.length; r++) {
    const row = rows[r];
    const idRaw = (row[idCol] || '').toString();
    const idCellNorm = idNorm(idRaw);
    console.log(`➡️ Revisando fila ${r+2}: id en hoja='${idRaw}' (norm='${idCellNorm}')`);

    if (idCellNorm === requestedNorm) {
      // Construir objeto con normalización de encabezados y serialización de valores
      const obj = {};
      headersKeys.forEach((key, i) => {
        obj[key] = serialize(rows[r][i]);
      });

      // TELEFONO: mapear y dejar solo últimos 7 dígitos (si existe)
      let phoneRaw = '';
      if (telefonoIdx !== -1) {
        phoneRaw = serialize(rows[r][telefonoIdx]);
      } else if (obj['telefono']) {
        phoneRaw = String(obj['telefono']);
      }
      phoneRaw = (phoneRaw || '').toString().replace(/\D/g, ''); // solo dígitos
      obj['telefono_completo'] = phoneRaw.length > 7 ? phoneRaw.slice(-7) : phoneRaw;

      // Normaliza: si el encabezado real es 'id', mapea también a id_registro
      if (obj['id'] && !obj['id_registro']) obj['id_registro'] = obj['id'];

      console.log(`✅ Registro encontrado en fila ${r+2} por coincidencia exacta.`);
      return { data: obj, row_index: r + 2 };
    }
  }

  // Fallback: intentamos búsqueda por inclusión (por si hay espacios/formatos distintos)
  for (let r = 0; r < rows.length; r++) {
    const row = rows[r];
    const idRaw = (row[idCol] || '').toString();
    const idCellNorm = idNorm(idRaw);

    if (idCellNorm.includes(requestedNorm) || requestedNorm.includes(idCellNorm)) {
      console.warn(`⚠️ Coincidencia parcial encontrada en fila ${r+2}: hoja='${idRaw}', solicitado='${idRegistro}'`);
      const obj = {};
      headersKeys.forEach((key, i) => { obj[key] = serialize(rows[r][i]); });

      // TELEFONO en fallback: mapear y dejar solo últimos 7 dígitos
      let phoneRaw = '';
      if (telefonoIdx !== -1) {
        phoneRaw = serialize(rows[r][telefonoIdx]);
      } else if (obj['telefono']) {
      ...
      phoneRaw = (phoneRaw || '').toString().replace(/\D/g, '');
      obj['telefono_completo'] = phoneRaw.length > 7 ? phoneRaw.slice(-7) : phoneRaw;
  ...
  console.log(`❌ getRegistroPorIdYFila: No se encontró registro para ID '${idRegistro}'`);
  return null;
}
...
function updateRegistro(idRegistro, nuevosDatos) {
  ...
  console.log("🔎 updateRegistro llamado con idRegistro:", idRegistro);
  console.log("📋 Encabezados normalizados:", headersNorm);
  console.log("📍 Columna ID detectada en índice:", idCol);

  // Buscar fila del registro
  const rows = sh.getRange(2, 1, lastRow - 1, lastCol).getValues();
  for (let r = 0; r < rows.length; r++) {
    const id = (rows[r][idCol] || '').toString().trim().toLowerCase();
    console.log(`➡️ Fila ${r+2}: ID en hoja = "${id}" comparando con "${idRegistro.trim().toLowerCase()}"`);

    if (id === idRegistro.trim().toLowerCase()) {
      const row = headers.map(h => {
        const key = norm(h);
        if (key === 'id') return idRegistro; // mantener el mismo ID
  ...
  console.log(`❌ No se encontró coincidencia para ID ${idRegistro}`);
